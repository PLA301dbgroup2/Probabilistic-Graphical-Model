...
los_gate_prompt() {
    ...
    echo "prior guide update, bert_prior, adr and gate" >> "$log_dir"
    

    python train_gated_prompt.py \
        --data_dir "$DATA_DIR" \
        --fold "$fold" \
        ...
        --task_desc "los_base_${dropout}_${learning_rate}" \
        --log_dir "$log_dir" \
        --device "0" \
        --use_prior \
        --use_guide \
...
for FOLD in 0 1 2 3 4; do 
    los_gate_prompt "$FOLD" "$CUDA_VISIBLE_DEVICES" "$MDP_DROPOUT" "$MDP_LR"
done
