...
def convert_tensor_to_list(data):
    ...
    if isinstance(data, torch.Tensor):
        ...
        return [convert_tensor_to_list(element) for element in data]
    elif isinstance(data, tuple):
        return tuple(convert_tensor_to_list(list(data)))
    else:
        return data
    ...
    if args.do_train:
        ...
        if args.do_prompt:
            ...
            if args.num_stacks == 2:
                ...
                temp_model = torch.load(os.path.join(os.getcwd(), f'saved_models/gated_prompt/{args.task_desc}_{args.fold}_final.pth'))
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for data, priors_data, bert_priors_data, kg_priors_data in zip(train_dataloader, train_priors_dataloader, bert_train_priors_dataloader, kg_train_priors_dataloader):
                ...
                dx_ints = data['dx_ints']
                proc_ints = data['proc_ints']


                max_indices_dx_ints = torch.max(dx_ints, dim=1).values
                max_indices_proc_ints = torch.max(proc_ints, dim=1).values


                max_index_dx_ints = torch.max(max_indices_dx_ints).item()
                max_index_proc_ints = torch.max(max_indices_proc_ints).item()



                bert_priors = {}
                bert_priors['indices'] = bert_priors_data[0].to(args.device)
                #print(bert_priors['indices'].shape)
                bert_priors['values'] = bert_priors_data[1].to(args.device)
                #print(bert_priors['values'].shape)
                kg_priors = {}
                kg_priors['indices'] = kg_priors_data[0].to(args.device)
                # print(kg_priors['indices'].shape)
                kg_priors['values'] = kg_priors_data[1].to(args.device)
                # print(kg_priors['values'].shape)
                all_priors = (priors_data, bert_priors, kg_priors)
                ...
                log_hot_list = convert_tensor_to_list(outputs[3])
                ...
                if global_step == 3000:
                    log_info = {
                        ...
                        'loss': loss.item(),  
                        'attentions': convert_tensor_to_list(outputs[3]) if len(outputs) > 3 else None,
                        'logits': convert_tensor_to_list(outputs[1]) if len(outputs) > 1 else None,
                    }
                    ...
                    with open(log_file, 'a', encoding='utf-8') as f:  
                        try:
                            ...
                            print(f"Unserializable object: {log_info}")

                    print(f"Log written at global_step {global_step}")
            ...
            # train_pbar.update(1)
            # if early_stop_steps >= args.early_stop_time:
            #     logging.info('*** training is stoped eraly ***')
            #     break
            if global_step >= max_steps:
                break
